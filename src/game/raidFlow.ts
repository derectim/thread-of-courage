...
export function getRaidStartStage(campaignResumeStage: number): number {
  return Number.isFinite(campaignResumeStage)
    ? Math.max(1, Math.floor(campaignResumeStage))
    : 1;
}
